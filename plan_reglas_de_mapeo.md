...
|---|---|
| §3, diagrama | Son la flecha menciones → ABox, y son **versionadas** |
| §3, regla de regeneración (D10 + §1.4) | "Se cambian las reglas de mapeo y se recomputa" — sustituyen al script de migración |
| §6.2 | "Revisar una decisión de fusión no requiere migración. Se cambia la regla de mapeo y se recomputa" |
| §6.4 | Notarizar, forzar y refutar viven en el nivel "ABox / reglas de mapeo", con alcance **por caso** |
| §6.8 | Tras aplicar una rama: "recomputar el ABox completo desde la capa de menciones con las reglas de mapeo actualizadas" |

Y una sexta referencia indirecta: el esquema de rama (§8.2) puntúa `abox_regen_cost: 340`,
...
versionan. La omisión es sistemática, no un párrafo olvidado: §8 define esquema para menciones,
...
- §6.4 le da alcance **por caso** a notarizar, forzar y refutar.
- §6.8 dice que la regeneración recomputa el ABox **entero**.
...
**Por qué el ancla y no otra cosa:** la capa de menciones es inmutable salvo por extensión (§3),
...
Satisface las dos restricciones a la vez: §7 quiere todo umbral en `config/default.yaml`, §6.4
...
- estado `open | accepted | rejected | superseded`, con lo rechazado sobreviviendo (§6.7);
...
de §6.4 y la pregunta por propiedad funcional de §6.8 entran como `kind` nuevos.
...
- El par `(state_hash, rules_hash)` identifica un ABox. Regenerar con el par ya visto es un
  no-op comprobable.
- `abox_regen_cost` (§8.2) se vuelve computable: es el diff entre el ABox del par actual y el
...
| `type_from` | Qué zonas de tipado producen `rdf:type` | `auto` — la zona gris no tipa hasta que la contestás. Es la política conservadora de §6.2 llevada al ABox |
| `provenance` | Cómo cada tripleta apunta a las menciones que la sostienen | Grafo con nombre por documento. Sin esto no hay procedencia verificable y §3 deja de sostenerse |
| `conflict_policy` | §6.4: `notarize` / `force` / `refute` | `notarize` — el único que no destruye información, y el default silencioso que el spec pide |
| `duplicate_policy` | Qué pasa con `possible_duplicate_unresolved` | Individuos separados, marcados, y **fuera del conteo de soporte funcional** (§6.8) |
...
- **Idempotente.** Correrla dos veces sobre el mismo par `(state_hash, rules_hash)` no cambia
  nada.
- **Total.** Nunca escribe en la capa de menciones. La flecha del diagrama de §3 va en un solo
...
Eso es lo que destraba el filtro SHACL de la cadena de validación (§6.6), que opera sobre el ABox y hoy no tiene sobre
qué operar.
...
- Reglas escritas por el modelo. Las reglas son decisiones del usuario o defaults del sistema;
  la extracción de candidatos (§6.1) ya prohíbe que el LLM escriba OWL, y esto es la misma
  frontera.
- Contextualizar (§6.4) — reificar una propiedad con su fuente. Es TBox, es por propiedad y no
