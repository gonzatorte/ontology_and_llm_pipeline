...
def store(tmp_path, zone=GREY, iri=CANDIDATE):
    ...
    conn.execute(
        ...
        "VALUES ('m1', 'd1', 1, 'focus group', 'extracted')"
    )
    conn.commit()
...
def test_an_unanswered_pair_is_waiting(tmp_path):
    conn = store(tmp_path)
    assert [row["mention_id"] for row in typing_store.pending(conn, "v1")] == ["m1"]


def test_an_answered_pair_leaves_the_queue(tmp_path):
    conn = store(tmp_path)
    typing_store.answer(conn, "m1", CANDIDATE)
    assert typing_store.pending(conn, "v1") == []


def test_an_automatic_typing_was_never_a_question(tmp_path):
    assert typing_store.pending(store(tmp_path, zone=AUTO), "v1") == []
...
def test_none_of_these_is_a_real_answer_and_orphans_the_mention(tmp_path):
    ...
    concept belongs."""
    conn = store(tmp_path)
    typing_store.answer(conn, "m1", None, offered=CANDIDATE)
    typing_store.persist_typings(conn, "v1", [Typing("m1", CANDIDATE, 0.86, GREY, None)])
...
def test_the_answer_may_name_a_different_class(tmp_path):
    conn = store(tmp_path)
    typing_store.answer(conn, "m1", "c:Interview", offered=CANDIDATE)
    typing_store.persist_typings(conn, "v1", [Typing("m1", CANDIDATE, 0.86, GREY, None)])
...
def test_taking_the_offer_is_an_accept_label(tmp_path):
    conn = store(tmp_path)
    typing_store.answer(conn, "m1", CANDIDATE, offered=CANDIDATE, score=0.86)
    label = typing_store.labels(conn)[0]
    assert label["accepted"] and label["surface_text"] == "focus group"


def test_refusing_the_offer_is_a_reject_label(tmp_path):
    conn = store(tmp_path)
    typing_store.answer(conn, "m1", None, offered=CANDIDATE, score=0.86)
    assert not typing_store.labels(conn)[0]["accepted"]
...
def test_no_answers_means_no_labels(tmp_path):
    assert typing_store.labels(store(tmp_path)) == []
